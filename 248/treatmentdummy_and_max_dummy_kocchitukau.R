...
# setwd() は環境依存のため、ここではコメントアウトします。
# ユーザーの環境でパスを設定してください。
# setwd("C:/Users/FMV/OneDrive - OUMail (Osaka University)/デスクトップ")

# --- ファイルパスの設定 ---
# '015/data/merge_final.csv' は適切なファイルパスに修正してください。
# スクリプトを実行するディレクトリからの相対パス、または絶対パスを指定してください。
...
# ----------------------------------------------------------------------
# 【修正箇所】: 正規表現を修正しました。
# '^T0|^TL|^TM|^TH' とすることで、participant.labelの先頭がT0, TL, TM, THのいずれかである場合にマッチします。
# ----------------------------------------------------------------------
df <- df %>%
  mutate(
...
# グループ（group_type と round n_id_subsession）ごとに、contributeとpayoffの最大値を
# 獲得したプレイヤーにフラグ(1)を立てます。
calculate_max_flags <- function(data, round_num) {
  ...
  data <- data %>%
    ...
    mutate(
      ...
      # contributeがグループ内最大値と等しい場合に1をセット
...
# max_c_and_p列の作成 (contributeとpayoffの両方が最大である場合に1)
for (i in 1:num_rounds) {
...
create_group_flag_columns <- function(data, group_names) {
  ...
  for (group in group_names) {
    ...
    data <- data %>%
      mutate(
        # group_type が該当するグループと一致する場合に1をセット
...
cat("\n--- 新規追加されたグループフラグ列の確認 (T0, TL, TM, TH) ---\n")
...
df_summary <- df2 %>%
  ...
  mutate(
    # group_type を factor に変換し、順序を指定する
